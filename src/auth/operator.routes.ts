    ...
    router.get('/perizie', verifyToken, async (req: Request, res: Response): Promise <any> => {
      try {
        ...
        const perizie = await Perizia.find({ codiceOperatore: userId }).select('codicePerizia dataOra coordinate indirizzo descrizione stato fotografie revisioneAdmin codiceOperatore');
